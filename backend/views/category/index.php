...
<div class="row">
    <div class="col-sm-3 col-md-2">
        <ul class="nav nav-sidebar">
            ...
            <li class="active"><a href="<?= Url::to(['index']) ?>">Category manage<span class="sr-only">(current)</span></a></li>
        ...
        <form action="<?=Url::to(['delete'])?>" method="post" id="ql" >
            ...
            <table class="table table-striped">
                ...
                <?php foreach ($model as $category):?>
                    <tr data-key="<?= Html::encode($category['id']) ?>">
                        <td><input type="checkbox" value="<?= Html::encode($category['id']) ?>" name="selection[]"></td>
                        <td><?= Html::encode($category['id']) ?></td>
                        <td><?= Html::encode($category['name']) ?></td>
                        <td><?= Html::encode($category['url_name']) ?></td>
                        <td><?= Yii::$app->formatter->format(Html::encode($category['create_time']), ['datetime', 'php: Y-m-d, H:i:s']) ?></td>
                        <td><?= Yii::$app->formatter->format(Html::encode($category['update_time']), ['datetime', 'php: Y-m-d, H:i:s']) ?></td>
                        <td>
                            <?=Url::remember();?>
                            <a class="glyphicon glyphicon-dashboard" title="View" href="<?=Url::to(['view']) . '?id=' . Html::encode($category['id']) ?>"></a>
                            <a class="glyphicon glyphicon-pencil" title="Update" href="<?=Url::to(['update']) . '?id=' . Html::encode($category['id']) ?>"></a>
                            <a class="glyphicon glyphicon-trash" title="Delete" data-actions="delete" href="<?=Url::to(['delete']) . '?id=' . Html::encode($category['id']) ?>"></a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
